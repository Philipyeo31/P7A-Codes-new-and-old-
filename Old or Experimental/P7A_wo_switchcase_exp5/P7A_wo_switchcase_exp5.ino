...
void loop()
{
  ...
  Step1();
  //Step2();
  Step3();  // step 3 needs to have have the initial "1ml" be removed since it is inaccurate. 
  Step4();
  Step5();
  Step7new();
  //WashSubAndDevTubing();
...
void lcdSetup() {
  ...
  tft.setCursor(0, 200);
  tft.setTextSize(2);
  tft.println("Previous Assay's Reading");
  tft.println(PrevData);
}
...
void Step2()
{
  ...
  delay(100); //og value for timer i 7200
  for (float timer = 0; timer < 5; timer ++) {
    tft.setCursor(0, 200);
...
void Step3()
{
  ...
  delay(2500);
  Serial.println("{p1:run,10}");//push remaining 1ml into reactor (og value is 30, 4.5)now 7 bc when p1 got error, it rotates 7 times gives us 950ul
...
void Step4()
{
  ...
  Serial.println("{p2:setspeed,-50}");
  delay(2500);
  Serial.println("{p2:run,2.35}");//loads 300microL into the reactor (1.91 original value, 2.35)
  delay(10000); 
...
void Step5()
{
  ...
  tft.setTextSize(2);
  tft.println("instantaneous reading is: ");
  averageReading = totalReading / 15.0;
  float avg = float(averageReading);
  //avg.concat("%");
  tft.setCursor(0, 200);
...
void Step7new()
{
  ...
  //to really empty out the impinger
  DisplayWashStep1c();
  Serial.println("{v1:on,A2}");
  delay(2500);
